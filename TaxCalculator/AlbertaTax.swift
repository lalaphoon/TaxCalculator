...
class AlbertaTax : ProvincialTax {
    ...
    func getRRSP(netIncome: Double, _ contribution: Double) -> (result: Double, process: [[String]]) {
        ...
        result = TP.foundation(vary, income, Location.Alberta.rawValue).result + TP.BasicPersonalAmount(income, contribution,Location.Federal,false) + TP.BasicPersonalAmount(income,contribution,Location.Alberta,false)
        
        var process : [[String]] = [["Net Income","","", TP.get2Digits(income)],
            ["Province/Territory","","",Location.Alberta.rawValue],
            ...
            ["Province/Territorial Tax","","",TP.get2Digits(TP.calculateTheDifference(vary, income, TP.ProvincialBracketDictionary[Location.Alberta]!))],
            ["Basic personal amount", Location.Alberta.rawValue, "", TP.get2Digits(TP.BasicPersonalAmount(income,contribution,Location.Alberta,false))],
    ...
    func getInterestIncome(netIncome: Double, _ interestIncome: Double) -> (result: Double, process: [[String]]) {
        ...
        var result =  TP.foundation(income, total, Location.Alberta.rawValue).result + TP.BasicPersonalAmount(income, interest ,Location.Federal,true) + TP.BasicPersonalAmount(income, interest,Location.Alberta,true)
        var process : [[String]] = [["Net Income","","", TP.get2Digits(income)],
            ["Province/Territory","","",Location.Alberta.rawValue],
            ...
            ["Province/Territorial Tax","","", TP.get2Digits(TP.calculateTheDifference(income, total, TP.ProvincialBracketDictionary[Location.Alberta]!))],
            ["Basic Personal Amount",Location.Alberta.rawValue,"",TP.get2Digits(TP.BasicPersonalAmount(income, interest,Location.Alberta,true))],
            ["Tax Payable", "","", TP.get2Digits(result)]]
        return (result, process)
