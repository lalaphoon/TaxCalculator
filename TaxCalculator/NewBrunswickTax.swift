//
//  NewBrunswickTax.swift
//  TaxCalculator
//
//  Created by Mengyi LUO on 2017-01-25.
//  Copyright © 2017 WTC Tax. All rights reserved.
//

import Foundation
//This guy has basic reduction
class NewBrunswickTax : ProvincialTax {
    var TP = TaxPro()
    var location = Location.New_Brunswick
    let profileProvince : String = Location.New_Brunswick.rawValue
    
    var BasicReduction_BaseAmount : Double = 16076
    var BasicReduction_Percentage : Double  = 0.03
    var BasicReduction_Threshold : Double = 624 //This one will be written in TP.BasicReduction
    ...
    func getForeignInvestmentIncome(_ netIncome: Double, foreignIncome: Double, foreignTaxPaid: Double, isUSStock: Bool) -> (result: Double, process: [[String]]) {
        ...
        var result = TP.foundation(income, total-Deduction_2012-Deduction_2011, profileProvince).result +
            ...
            getBasicReduction(income, total - Deduction_2011 - Deduction_2012)
        ...
        var process = [["Net Income","","", TP.get2Digits(income)],
            ...
            ["Basic Reduction", location.rawValue,"",TP.get2Digits(getBasicReduction(income, total - Deduction_2011 - Deduction_2012 ))],
    ...
    func getBasicReduction(_ A: Double, _ B: Double, _ special: Bool = false , _ di: Double = 0.0) -> Double {
        return getSingleReduction(A) - getSingleReduction(B,special,di)
    }
    
    func getSingleReduction(_ val : Double, _ special: Bool = false, _ di : Double = 0.0) -> Double{
        var result = 0.0
        ...
        b = a - b
        var c : Double = val - self.BasicReduction_BaseAmount // where is 19000 coming from? where is 3.5% coming from?
        if c <= 0 {
            c = 0
        }
        var d = c * self.BasicReduction_Percentage
